		...
		match &self.value {
			...
			MPNodeType::List(ns) => {
				...
				for (ind,n) in ns.iter().enumerate() {
					if let MPNodeType::String(_) = &n.value {
						if tc != StackStringUsage::Unknown {
							result.push((StackStringUsage::CompiledCASText(ind),n.clone()));
						} else {
							result.push((StackStringUsage::ListElement(ind),n.clone()));
						}
					} else if tc != StackStringUsage::Unknown {
...
impl StackMaximaParser {
	...
	pub fn parse(&mut self, source: String) -> Option<MPNode> {
        ...
        let mut kreverttoken: Option<StackMaximaToken> = None;
        let mut krevertstack: Vec<ParserStackItem> = Vec::new();
        let mut krevertreset: i32 = 0;
		...
		loop {
				...
				while token.is_some() {
					if let StackMaximaParserTokenType::WS(_) = token.clone().unwrap().value {
						...
						if self.collect_comments {
							comments.push(
								MPNode::new(
									MPPosition::new_t(&token.clone().unwrap()),
				...
				if token.is_none() {
					...
					match token.clone().unwrap().value {
    					...
    					StackMaximaParserTokenType::SYMBOL(content) => {
    						if content == "^".to_string() || content == "^^".to_string() || content == "**".to_string() {
    							// Tricky synthetic tokens for power + unary op cases.
    							let mut lookahead: Vec<StackMaximaToken> = Vec::new();
    							let mut next: Option<StackMaximaToken> = self.lexer.next_token();
	   							while next.is_some() {
									if let StackMaximaParserTokenType::WS(_) = next.clone().unwrap().value {
										lookahead.push(next.clone().unwrap());
								...
								if next.is_some() {
									if let StackMaximaParserTokenType::SYMBOL(op2) = next.clone().unwrap().value {
										if op2 == "+".to_string() || op2 == "-".to_string() || op2 == "+-".to_string() || op2 == "#pm#".to_string() {
											token = Some(token.unwrap().merge(next.unwrap()));
											sym = format!(" {}{}", content, op2);
										} else {
											// Return the unused future.
											sym = content;
											self.lexer.return_token(next.unwrap());
											next = lookahead.pop();
											while next.is_some() {
												self.lexer.return_token(next.unwrap());
												next = lookahead.pop();
									...
									// If None the end is near and there is nothing worth
									// looking at in the future. No point in returning the future.
									sym = content;
								}
			...
			let mut current_state: usize = match stack[stack.len()-1] {
			...
			// If we have a keyword, we need to consider the possibility that it is an id.
			// So lets setup a revert point.
			if token != None {
				if let StackMaximaParserTokenType::KEYWORD(..) = token.clone().unwrap().value {
					if parsertables.get_action(current_state, "ID".to_string()) != TableAction::None {
						krevertstack = stack.clone();
						kreverttoken = token.clone();
						krevertreset = 3;
					}
				}
			}
			...
			if action == TableAction::None {
				for attempt in &self.inserts_to_attempt {
					...
					if let StackMaximaParserTokenType::SYMBOL(a) = &attempt.value {
						...
						let maybe_action: TableAction = parsertables.get_action(current_state, "END TOKEN".to_string());
						if maybe_action != TableAction::None {
							self.lexer.return_token(token.clone().unwrap());
							sym = "END TOKEN".to_string();
							let mut itoken = attempt.clone();
						...
						let maybe_action: TableAction = parsertables.get_action(current_state, "LIST SEP".to_string());
						if maybe_action != TableAction::None {
							self.lexer.return_token(token.clone().unwrap());
							sym = "LIST SEP".to_string();
							let mut itoken = attempt.clone();
				...
				// Maybe KEYWORD -> ID?
				if action == TableAction::None && kreverttoken != None {
					self.lexer.return_token(token.unwrap());
					let mut ntoken = kreverttoken.unwrap();
					if let StackMaximaParserTokenType::KEYWORD(ref v,_) = ntoken.value {
						ntoken.value = StackMaximaParserTokenType::ID(v.clone());
					}
					token = Some(ntoken);
					sym = "ID".to_string();
					stack = krevertstack;
					krevertstack = Vec::new();
					kreverttoken = None;
					krevertreset = 0;
					current_state = match stack[stack.len()-1] {
						ParserStackItem::State(state) => {
							state
						}
						_ => {
					...
					action = parsertables.get_action(current_state, sym.clone());
				}
			...
			if let TableAction::Shift(target_state) = action {
				...
				match rule {
					...
					113..=124 => {
						let _ = stack.pop();
						let mut term2mn = if let ParserStackItem::Node(a) = stack.pop().unwrap() {a} else {panic!("Unexpected item in stack.");};
						let _ = stack.pop();
						let term1t = if let ParserStackItem::Token(a) = stack.pop().unwrap() {a} else {panic!("Unexpected item in stack.");};
						let _ = stack.pop();
						let term0n = if let ParserStackItem::Node(a) = stack.pop().unwrap() {a} else {panic!("Unexpected item in stack.");};
						let (op1,op2) = term1t.split();
						if let StackMaximaParserTokenType::SYMBOL(symbol2) = op2.value.clone() {
							term2mn = MPNode::new(MPPosition::new_tn(&op2, &term2mn), MPNodeType::PrefixOperation(symbol2, Box::new(term2mn)))
						} else {
							panic!("Unexpected token type.")
						};
						if let StackMaximaParserTokenType::SYMBOL(symbol1) = op1.value.clone() {
							ParserStackItem::Node(MPNode::new(MPPosition::new_nn(&term0n, &term2mn), MPNodeType::Operation(Box::new(term0n), symbol1, Box::new(term2mn), InsertionType::None)))
						} else {
							panic!("Unexpected token type.")
						}
					},
					...
					62 | 67 | 103..=104 => {
						...
						let _ = stack.pop();
						let term0t = if let ParserStackItem::Token(a) = stack.pop().unwrap() {a} else {panic!("Unexpected item in stack.");};
						if let StackMaximaParserTokenType::KEYWORD(symbol,_) = term0t.value.clone() {
							ParserStackItem::Node(MPNode::new(MPPosition::new_tn(&term0t, &term1n), MPNodeType::PrefixOperation(symbol, Box::new(term1n))))
						} else {
							panic!("Unexpected token type.")
						}
					...
					81..=84 | 95..=101 | 105..=107 | 112 => {
						...
						let _ = stack.pop();
						let term0n = if let ParserStackItem::Node(a) = stack.pop().unwrap() {a} else {panic!("Unexpected item in stack.");};
						if let StackMaximaParserTokenType::KEYWORD(symbol,_) = term1t.value.clone() {
							ParserStackItem::Node(MPNode::new(MPPosition::new_nn(&term0n, &term2n), MPNodeType::Operation(Box::new(term0n), symbol, Box::new(term2n), InsertionType::None)))
						} else {
							panic!("Unexpected token type.")
						}
